...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  default_random_engine random_num_engine;
	...
	for (int i = 0; i < num_particles; ++i) {
		Particle p = { i, normal_dist_x_init(random_num_engine), normal_dist_y_init(random_num_engine), normal_dist_theta_init(random_num_engine), 1.0 };
...
void ParticleFilter::prediction(double delta_t, double std_pos[], double velocity, double yaw_rate) {
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    // Make predictions of and check 0 values
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for (int i = 0; i < num_particles; ++i) {
    ...
    for (int j = 0; j < observations.size(); j++) {
      ...
      double obs_x = particle.x + observations[j].x * cos(particle.theta) - observations[j].y * sin(particle.theta);
      double obs_y = particle.y + observations[j].x * sin(particle.theta) + observations[j].y * cos(particle.theta);
      ...
      for (int k = 0; k < map_landmarks.landmark_list.size(); k++) {
        ...
        double dist_part2lm = dist(particle.x, particle.y, map_landmarks.landmark_list[k].x_f, map_landmarks.landmark_list[k].y_f);
        
        if (dist_part2lm <= sensor_range) {
          
          double dist_obs2lm = dist(obs_x, obs_y, map_landmarks.landmark_list[k].x_f, map_landmarks.landmark_list[k].y_f);
      ...
      // obs calculation using multivariate Gaussian
      double obs_w = exp(-0.5 * ((pow(delta_x, 2) / pow(std_lm_x, 2)) + (pow(delta_y, 2) / pow(std_lm_y, 2)))) / (2 * M_PI * std_lm_x * std_lm_y);
      weight = weight * obs_w;
